...
mlir::LogicalResult IslScop::create_line_number_map() {
  // this is already done while creating the isl read/write relations
  return success();
}
...
mlir::LogicalResult IslScop::create_bullseye_data() {
...
void IslScop::dump_line_number_map(llvm::raw_ostream &o) {
  o << "\n\nLine map dump:";
  for (auto it : access_to_line_number_map) {
    o << "\n" << it.first << " : " << it.second;
  }
}
